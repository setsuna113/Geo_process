#!/usr/bin/env python3
"""
Full end-to-end pipeline test with automatic cleanup.
"""
...
def create_test_datasets():
    ...
    try:
        ...
        print("   ⚠️ rasterio not available, using original datasets")
...
def run_pipeline_test():
    ...
    try:
        ...
        results = pipeline.run_complete_pipeline(
            experiment_name=f"test_full_pipeline_{timestamp}",
            description="Full pipeline test with small datasets",
            skip_existing=False
        )
        ...
        if 'som_analysis' in results:
            print(f"      SOM analysis: {results['som_analysis']['saved_path']}")
    ...
    try:
        ...
        with db.get_connection() as conn:
            ...
            if cur.fetchone()[0]:
                ...
                cur.execute("SELECT data_table_name FROM resampled_datasets WHERE name LIKE 'test-%' OR name IN ('plants-richness', 'terrestrial-richness')")
                tables_to_drop = [row[0] for row in cur.fetchall() if row[0]]
                
                # Delete records
                cur.execute("DELETE FROM resampled_datasets WHERE name LIKE 'test-%' OR name IN ('plants-richness', 'terrestrial-richness')")
...
def main():
    print("🧪 Unified Resampling Pipeline - Full Test")
    print("=" * 50)
    
    # Run the test
    success, results = run_pipeline_test()
    ...
    if success:
        ...
        if results and 'merged_dataset' in results:
            print("   ✅ Datasets were merged")
        if results and 'som_analysis' in results:
